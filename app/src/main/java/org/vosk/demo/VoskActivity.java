...
public class VoskActivity extends Activity implements RecognitionListener {
    ...
    private Lcs check() {
        ...
        //全部转换为小写
        try {
            sentence = new ConverterUtils().fileTosentence(txtName);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    */
    ...
    private void show(){
        ...
        for (int i = 0; i < sentence_splited.size(); i++) {
            SpannableString msp = msp = new SpannableString(sentence_splited.get(i)+" ");
        ...
        score+=(double) (answerFirst.size())*100/sentence_splited.size();
